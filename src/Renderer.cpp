...
void Renderer::renderFrame()
{
    ...
    x *= size.x;
    y *= size.y;
    this->subFrame = sf::IntRect(sf::Vector2i(x, y), sf::Vector2i(size.x, size.y));
    ...
    oldSprite.setPosition((sf::Vector2f)this->subFrame.getPosition());
...
{
    if (!this->targetTexture->getTexture().copyToImage().saveToFile(name))
    {
...
int Renderer::getPixelDifference() const
{
    ...
    for (int x = 0; x < this->size.get().x; x++)
    {
        ...
        {
            sf::Vector2u coord(x, y);
            if (image1.getPixel(coord) != image2.getPixel(coord))
            {
                result++;
            }
        }
